function[Eii_dev,Eii_median] = median_dev_Eii(Eii_xymap,grain_D,grain_comp,listDB)
...
ip = nan(1,N);                                 % LOCAL index (1 to n) of phases used to extract flow parameters
...
for i = 1:N
    if strcmp(listDB(grain_comp(i)),'not defined') ~= 1
        % calculate grain surface molar proportion as molar volumetric proportion*area_grain/total_area
        mgs(i) = alpha(ip(i))*(pi*(grain_D(i)/2)^2)/S;   
    end
end
% compute grain strainrate averaged on molar proportions
Eiim = 10.^Eii_xymap.*mgs;
% compute 'central' strainrate
Eii_median = log10(sum(Eiim,'omitnan'));
Eii_dev = (Eii_xymap-Eii_median)/Eii_median*100;
end
...
%% (5)  MORE
m5 = uimenu(f,'Label','More');
uimenu(m5,'Label','High Quality Plot Export','Callback',{@export_high_quality_plot});
uimenu(m5,'Label','Export Stress and Strain Data','Callback',{@export_stress_strain_data});
...
function select_phase(~,~)
    ...
    if strcmp(get(gcbo, 'Checked'),'on')
        ...
        set(gcbo, 'Checked', 'on');
        phase_ID = find(strcmp(listDB, 'quartz')); % Set quartz as default
...
function reset_rheology(~,~)
    % Reset all rheological parameters and handle menu items safely
    ...
    switch reset_rheology
        case 'Yes'
            ...
            Sii        = [];
            SiiEiiMap  = [];
            % Safely update menu items by checking validity first
            % Handle Voigt/Reuss selection
            for i = 6:7
                if isvalid(m1h(i)) && isa(m1h(i), 'matlab.ui.container.Menu')
                    set(m1h(i), 'Checked', 'off', 'Enable', 'off');
                end
            end
            ...
            for i = 1:4
                if isvalid(m1h(i)) && isa(m1h(i), 'matlab.ui.container.Menu')
                    set(m1h(i), 'Checked', 'off', 'Enable', 'on');
                end
            end
            
            % Handle mixture rules
            for i = 9:13
                if isvalid(m1h(i)) && isa(m1h(i), 'matlab.ui.container.Menu')
                    set(m1h(i), 'Checked', 'off', 'Enable', 'off');
                end
...
function display_grainsize_stats(mapgrain_D)
    % Calculate and display grain size statistics
    grain_sizes = cellfun(@mean, mapgrain_D);
    mean_grain_size = mean(grain_sizes);
    median_grain_size = median(grain_sizes);
    std_grain_size = std(grain_sizes);
    disp(['Mean grain size: ', num2str(mean_grain_size)]);
    disp(['Median grain size: ', num2str(median_grain_size)]);
    disp(['Standard deviation: ', num2str(std_grain_size)]);
end
...
function export_high_quality_plot(~,~)
    % Export the central plot as a high-quality PNG image
    [file, path] = uiputfile('*.png', 'Save High Quality Plot As');
    if isequal(file, 0) || isequal(path, 0)
        return; % User canceled the file dialog
    end
    fullFileName = fullfile(path, file);
    % Set the DPI for high-quality export
    dpi = 900; % Increase DPI for better quality
    % Export the central plot
    centralPlot = findobj(f, 'Type', 'axes');
    if ~isempty(centralPlot)
        exportgraphics(centralPlot(1), fullFileName, 'Resolution', dpi, 'ContentType', 'vector');
        msgbox('High quality plot exported successfully!', 'Export Complete');
    else
        msgbox('No plot found to export.', 'Export Error');
    end
end

function export_stress_strain_data(~,~)
    try
        [file, path] = uiputfile('*.xlsx', 'Save Stress/Strain Data As');
        if isequal(file, 0) || isequal(path, 0)
            return;
        end
        fullFileName = fullfile(path, file);
        ...
        % Initialize variables
        median_strain_rate = NaN;
        max_strain_rate = NaN;
        peak_stress = NaN;
        max_stress = NaN;
        
        % Format headers based on mechanical constraint
        if VR == 1 % Isostrain (Voigt)
            const_type = 'Isostrain (Voigt)';
            if ~isempty(Eii_xymap)
                median_strain_rate = Eii; % Use prescribed strain rate
                max_strain_rate = Eii;
            end
            if ~isempty(Sii_xymap)
                peak_stress = max(Sii_xymap(:));
                max_stress = max(max(Sii_xymap));
            end
        else % Isostress (Reuss)
            const_type = 'Isostress (Reuss)';
            if exist('Sii', 'var') && ~isempty(Sii) % Fixed condition check
                peak_stress = Sii; % Use prescribed stress
                max_stress = Sii;
            end
            if ~isempty(Eii_xymap)
                median_strain_rate = median(Eii_xymap(:));
                max_strain_rate = max(max(Eii_xymap));
            end
        end

        % Format strain rate values
        if exist('Eii', 'var') && ~isempty(Eii)
            formatted_strain_rate = sprintf('%.2e', Eii);
        else
            formatted_strain_rate = '';
        end
        
        % Format median and max rates
        if ~isnan(median_strain_rate)
            formatted_median_rate = sprintf('%.2e', median_strain_rate);
        else
            formatted_median_rate = '';
        end
        
        if ~isnan(max_strain_rate)
            formatted_max_rate = sprintf('%.2e', max_strain_rate);
        else
            formatted_max_rate = '';
        end
        ...
        % Create headers with appropriate units
        headers = {'Phase Name', 'Grain Size (um)', 'Grain Size Error', 'Temperature (C)', ...
            'Temperature Error', 'Applied Stress (MPa)', 'Applied Strain Rate (s^-1)', ...
            'Median Strain Rate (s^-1)', 'Max Strain Rate (s^-1)', ...
            'Peak Stress (MPa)', 'Max Stress (MPa)', 'Mechanical Constraint'};
        
        % Create data row with formatted values and appropriate units
        data_row = {
            char(phase_name{1}), ...       % Phase name
            cgs*1000, ...                  % Grain size (um)
            0, ...                         % Grain size error
            actual_temp, ...               % Temperature (C)
            0, ...                         % Temperature error
            Sii, ...                       % Applied stress (MPa)
            formatted_strain_rate, ...      % Applied strain rate (s^-1)
            formatted_median_rate, ...      % Median strain rate (s^-1)
            formatted_max_rate, ...         % Max strain rate (s^-1)
            peak_stress, ...               % Peak stress (MPa)
            max_stress, ...                % Max stress (MPa)
            const_type                     % Mechanical constraint type
        };
        
        % Write data
        output_data = [headers; data_row];
        writecell(output_data, fullFileName, 'Sheet', 1);
        ...
        msgbox('Stress/Strain data exported successfully!', 'Export Complete');
    catch ME
        errordlg(['Error during export: ' ME.message], 'Export Error');
